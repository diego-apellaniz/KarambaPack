...
namespace KarambaPack
{
    ...
    {
        public static Dictionary<Tuple<int, int>, double> dictLFactors(List<string> Combos)
        //Extract the corresponding Load Factors from the definition of the Load Combinations
        {
            //Dictionary((LCombo, LCase), LFactor)
            var LFactors = new Dictionary<Tuple<int, int>, double>();
            int LCindex;
            double factor;
            for (int i = 0; i < Combos.Count; i++)
            {
                ...
                for (int j = 0; j < parts1.Length; j++)
                {
                    if (parts1[j].Length > 0)
                    {
                        ...
                        LCindex = int.Parse(parts2[1]);
                        LFactors.Add(new Tuple<int, int>(i, LCindex), factor);
                    }
                }
            }
            return LFactors;
